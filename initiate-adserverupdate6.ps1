...
$remoteInfoScript = {
    ...
    try {
        ...
        $lastReboot = if ($os.LastBootUpTime) { ([Management.ManagementDateTimeConverter]::ToDateTime($os.LastBootUpTime)).ToString("yyyy-MM-dd HH:mm:ss") } else { "N/A" }
    } catch { $lastReboot = "N/A" }
    ...
    try {
        ...
        if ($os) {
            ...
            Write-Host "Method 1 (CIM) succeeded"
        }
    } catch {
        Write-Host "Method 1 (CIM) failed: $($_.Exception.Message)"
    }
    ...
    if ($winVer -eq "Unknown") {
        try {
            ...
            if ($productName -and $currentBuild) {
                ...
                Write-Host "Method 2 (Registry) succeeded"
            }
        } catch {
            Write-Host "Method 2 (Registry) failed: $($_.Exception.Message)"
        }
    }
    ...
    if ($winVer -eq "Unknown") {
        try {
            ...
            if ($os) {
                $winVer = "$($os.Caption) Build $($os.BuildNumber)"
                Write-Host "Method 3 (WMI) succeeded"
            }
        } catch {
            Write-Host "Method 3 (WMI) failed: $($_.Exception.Message)"
        }
    }
    ...
    if ($winVer -eq "Unknown") {
        try {
            ...
            Write-Host "Method 4 (Environment) succeeded"
        } catch {
            Write-Host "Method 4 (Environment) failed: $($_.Exception.Message)"
        }
    }
    ...
    if ($winVer -eq "Unknown") {
        try {
            ...
            if ($sysInfo) {
                ...
                Write-Host "Method 5 (systeminfo) succeeded"
            }
        } catch {
            Write-Host "Method 5 (systeminfo) failed: $($_.Exception.Message)"
        }
    }
